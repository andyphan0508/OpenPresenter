...
export function DisplaySettings() {
  ...
  return (
    <div className="grid grid-cols-2 gap-5">
      <Field label="Màu nền khi trống">
        {(id) => (
          <ColorInput
            id={id}
            value={output.backgroundColor}
            onChange={(backgroundColor) =>
              updateOutputSettings({ backgroundColor })
            }
          />
        )}
      </Field>
            ...
            onChange={(e) =>    
              e.target.value === "off"
                ...
                : updateOutputSettings({
                    showClock: true,
                    clockPosition: e.target
                      .value as OutputSettings["clockPosition"],
                  })
            }
            className="input"
          >
      ...
      <Field label="Theme mặc định cho bài hát">
        {(id) => (
          <select
            id={id}
            value={settings.songThemeId}
            onChange={(e) => updateSettings({ songThemeId: e.target.value })}
            className="input"
          >
            {themes.map((t) => (
      ...
      <Field label="Theme mặc định cho Kinh Thánh">
        {(id) => (
          <select
            id={id}
            value={settings.bibleThemeId}
            onChange={(e) => updateSettings({ bibleThemeId: e.target.value })}
            className="input"
          >
            {themes.map((t) => (
              <option key={t.id} value={t.id}>
      ...
      <p className="col-span-2 text-xs text-muted">
        Màn hình khán phòng mở trên màn hình phụ thứ nhất, màn hình sân khấu
        trên màn hình phụ thứ hai (nếu có).
      </p>
    </div>
  );
}
