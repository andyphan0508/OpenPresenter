    ...
    <div className="flex flex-col h-full bg-panel">
      ...
      <div className="px-3 py-2.5 border-b border-app flex items-center justify-between flex-shrink-0">
        ...
        <button
          ...
          className="text-xs bg-orange-500 hover:bg-orange-600 text-white px-2.5 py-1 rounded transition-colors font-medium"
        >
          + New
        </button>
      </div>

      {/* Inline new presentation form */}
      {showNew && (
        <div className="p-3 border-b border-app bg-surface">
          ...
          <div className="flex gap-1.5">
            <button
              ...
              className="flex-1 py-1 text-xs rounded bg-gray-100 dark:bg-[#2a2a2a] text-muted hover:text-primary transition-colors"
            >
              Cancel
            </button>
            <button
              onClick={handleCreate}
              className="flex-1 py-1 text-xs rounded bg-orange-500 text-white hover:bg-orange-600 font-medium transition-colors"
            >
              Create
            </button>
          </div>
        ...
        {presentations.length === 0 ? (
          <div className="p-4 text-center">
            <div className="w-10 h-10 rounded-lg border-2 border-dashed border-gray-200 dark:border-[#333] flex items-center justify-center mx-auto mb-3">
              <span className="text-gray-300 dark:text-[#444] text-lg">▣</span>
            </div>
            <p className="text-muted text-sm font-medium mb-1">No presentations yet</p>
            <p className="text-faint text-xs mb-3 leading-relaxed">
              Create one to start adding slides
            </p>
            <button
              onClick={() => setShowNew(true)}
              className="text-xs bg-orange-500 hover:bg-orange-600 text-white px-3 py-1.5 rounded transition-colors font-medium"
            >
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-medium truncate ${
                      currentPresentationId === pres.id ? 'text-primary' : 'text-secondary'
                    }`}>
                      {pres.name}
                    </p>
                  ...
                  <div className="hidden group-hover:flex gap-0.5 flex-shrink-0">
                    <button
                      ...
                      className="w-6 h-6 rounded bg-gray-100 dark:bg-[#2a2a2a] hover:bg-gray-200 dark:hover:bg-[#333] text-muted hover:text-primary text-xs flex items-center justify-center transition-colors"
                      title="Rename"
                    >
                      ✎
                    </button>
                    <button
                      onClick={(e) => {
                        e.stopPropagation()
                        if (confirm(`Delete "${pres.name}"? This cannot be undone.`)) {
                          deletePresentation(pres.id)
                        }
                      }}
                      className="w-6 h-6 rounded bg-gray-100 dark:bg-[#2a2a2a] hover:bg-red-500 hover:text-white text-muted text-xs flex items-center justify-center transition-colors"
                      title="Delete"
                    >
                      ✕
                    </button>
