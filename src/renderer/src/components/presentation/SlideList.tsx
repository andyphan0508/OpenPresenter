...
import { Play, StickyNote, Copy, Trash2, Plus } from 'lucide-react'
...
function SlideThumb({ slide, index, isActive, isLive, onClick, onDelete, onDuplicate }: SlideThumbProps) {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 rounded overflow-hidden" style={bgStyle}>
        {bg.type === 'video' && (
          <div className="absolute inset-0 flex items-center justify-center">
            <Play className="w-6 h-6 text-white opacity-50" fill="currentColor" />
          </div>
        )}
      ...
      {slide.notes && <StickyNote className="absolute top-0.5 left-1 w-2.5 h-2.5 text-yellow-400/70" />}
      ...
      <div className="absolute top-0.5 right-0.5 hidden group-hover:flex gap-0.5 z-10">
        <button
          ...
          className="w-5 h-5 bg-black/50 hover:bg-black/70 rounded text-white flex items-center justify-center"
          title="Duplicate"
        >
        ...
        <button
          ...
          className="w-5 h-5 bg-black/50 hover:bg-red-600 rounded text-white flex items-center justify-center"
          title="Delete"
        >
...
export function SlideList({ onGoLive }: SlideListProps) {
  ...
  const handleDrop = (toIdx: number) => {
    ...
    setDragOverIdx(null)
    if (from === null || !currentPresentationId || from === toIdx) return
    moveSlide(currentPresentationId, from, toIdx)
  }
      ...
      >
        {pres.slides.map((slide, idx) => (
          <div
            ...
            onDragOver={(e) => {
              e.preventDefault()
              if (dragIndexRef.current !== null && dragOverIdx !== idx) setDragOverIdx(idx)
            }}
            ...
            className={`rounded transition-all ${
              dragOverIdx === idx ? 'ring-2 ring-orange-400 scale-105' : ''
            }`}
          >
            <SlideThumb
              slide={slide}
        ...
        <button
          ...
          style={{ aspectRatio: '16/9' }}
        >
          <Plus className="w-6 h-6 text-muted group-hover:text-orange-500 transition-colors" />
        </button>
      </div>
    </div>
  )
